...
@section('title_template')
    <title>WisataLampung | Data Kategori Wisata</title>
@endsection
{{-- end title --}}
        ...
        <table class="table table-report table-report--bordered display datatable w-full">
            <thead>
                <tr>
                    ...
                    <th class="border text-center whitespace-no-wrap">Nama Kategori Wisata</th>
                    <th class="border text-center whitespace-no-wrap">Actions</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($result->data->kategoriWisata as $output)
                    <tr class="border">
                        <td class="text-center border">{{ $loop->iteration }}</td>
                        <td class="text-center border">{{ $output->id }}</td>
                        <td class="text-center border">{{ $output->nama_kategori }}</td>
                        <td class="border w-5">
